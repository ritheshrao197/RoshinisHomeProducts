...
export default function OrdersPage() {
    const [orders] = useState([
        { id: 'ORD-1001', customer: 'Sushma R.', method: 'PhonePe', amount: 450, status: 'Processing', date: 'Oct 24, 2023 14:30' },
        { id: 'ORD-1002', customer: 'Praveen K.', method: 'COD', amount: 1200, status: 'Shipped', date: 'Oct 24, 2023 10:15' },
        { id: 'ORD-1003', customer: 'Ananya S.', method: 'PayU', amount: 250, status: 'Pending', date: 'Oct 23, 2023 18:45' },
        { id: 'ORD-1004', customer: 'Kiran M.', method: 'PhonePe', amount: 800, status: 'Delivered', date: 'Oct 22, 2023 09:20' },
    ]);
                        ...
                        <tbody className="divide-y divide-earth-100 text-sm text-earth-600">
                            {orders.map(order => (
                                <tr key={order.id} className="hover:bg-earth-50/50">
                                    ...
                                    <td className="px-6 py-4 text-earth-500">{order.date}</td>
                                    <td className="px-6 py-4">{order.customer}</td>
                                    <td className="px-6 py-4 text-earth-500">{order.method}</td>
                                    <td className="px-6 py-4 font-medium">₹{order.amount}</td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${order.status === 'Delivered' ? 'bg-green-100 text-green-800' :
